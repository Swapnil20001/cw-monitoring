...
class ServicesAlarmChecker:
    ...
    def alarm_json_metrics(self, alarm_configuration, alarms_json, metric, resource_name,engine=False):
        if not engine:
    ...
    def validation(self, alarm, alarm_configuration, alarmconfig, alarmname,resource_name,engine=False):
        if not engine:
            validation = {
                'alarmname': alarmname,
    ...
    # def rds_alarm_json_metrics(self, alarm_configuration, alarms_json, metric, resource_name,engine):
    #     alarms_json[metric].append({
    #         'alarmname': ' ',
    #         'alarmconfig': False,
    #         'Validation': 'fail',
    ...
    # def rds_validation(self, alarm, alarm_configuration, alarmconfig, alarmname,resource_name, engine):
    #     rds_validation = {
    #         'alarmname': alarmname,
    #         'alarmconfig': alarmconfig,
    #         'Validation': 'pass',
    ...
    def get_service_info(self, service_type, instance_id, resource_name, metrics, engine=False):
        if service_type == 'EC2':
    ...
    # def rds_get_service_info(self, service_type, instance_id, resource_name, metrics, engine):
    #     if service_type == 'RDS':
    #         return self.obj_rds.check_rds_alarms(instance_id, resource_name, metrics,engine)
    #     else:
    #         logger.error("Invalid service name: %s", service_type)
    ...
    # def generate_yaml_reports_rds(self, resources_data, instance_dict, check_alarms_func):
    #     result = {}
    #     for resource in resources_data:
    #         resource_id = resource['ResourceID']
    #         resource_name = resource['ResourceName']
            ...
            for instance_id, metrics in instances.items():
                for resource in metrics.values():
                    ...
                    if service_type =="RDS":
                        ...
                        # print(engine)
                        service_info = self.get_service_info(service_type, instance_id, resource_name, metrics,engine=engine)
                    else:
